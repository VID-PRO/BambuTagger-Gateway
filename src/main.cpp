...
// Port 8883 is handled by MqttBridge as a TLS-wrapped MQTT server using the
// self-signed cert (with Bambu OIDs) generated by certgen. Port 1883 is
...
static void manageAP() {
  ...
  static bool apStopped = false;

  if (staConnected && !apStopped && mode == WIFI_AP_STA) {
    WiFi.softAPdisconnect(true);
    WiFi.mode(WIFI_STA);
    apStopped = true;
    Serial.println("AP stopped (station connected)");
  } else if (!staConnected && mode == WIFI_STA && strlen(webconfigGetConfig()->stationSsid) > 0) {
    apStopped = false;
...
void setup() {
  ...
  camProxy.begin();
  ftpsProxy.begin();
  Serial.printf("MQTT TLS server on port %d\n", MQTT_PRINTER_PORT);
...
void loop() {
  ...
  mqtt.loop();  // handles both port 1883 (plain) and 8883 (TLS)
  camProxy.loop();
  ftpsProxy.loop();
  ArduinoOTA.handle();
  ledLoop();
